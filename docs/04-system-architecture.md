...
- 任务标题、统一正文、父子关系、Owner、状态、截止日期、标签和仅影响外观的展示类型。
- 任务编号分配。
- 完成、重新打开、移动和归档。
- 子任务统计投影。
...
- 同级边增删。
- 环检测和拓扑排序。
- 普通模式下按“拥有任一端点或两者共同直接父任务”授权依赖修改；顶层任务由 Project Owner 控制虚拟项目根作用域。
...
- 允许无确认读取全部授权任务信息；任何任务管理数据修改必须进入提案确认流程。
- 为当前用户有写资格且持有租约的任一类型工作区提供完整文件访问；进入管理员模式必须显式确认，退出可以自动执行。
...
- 完成摘要任务、Wiki 投影和全文检索。
- 评论。
- Owner 变更、评论、阻塞、依赖、截止日期和父任务“可完成”通知。
...
- 当前任务的直接子任务显示为独立列表，列表中呈现同级依赖、状态、Owner、截止日期和子任务统计。
...
- Agent 任务修改：所有提案与确认绑定目标版本；版本变化后确认失效。对当前用户有写资格的三种工作区文件操作由各自租约和同步版本约束，不使用任务修改确认。
