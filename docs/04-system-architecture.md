...
实现同步（2026-07-31）：正式数据库 profile 仍为 version 3，最新迁移为 `0010-m3-task-ui-history-compatibility`；M3 已在 M2 Fastify `/api/v1`/OpenAPI、共享领域/Repository、评论/活动/通知、审计/Worker/SSE 和 Task Workspace 原子边界之上交付生产 React Task UI，并保持 M1 中文治理 Web 与精确 prototype 入口兼容。
...
正式 Schema profile version 3 的最新迁移是 `0010-m3-task-ui-history-compatibility`。该前向迁移解除不可变完成快照对已删除活动 Task 行的外键约束，同时保留 Task UUID、项目和 Workspace 历史。数据库保存：
...
M3 当前公共表面在 M2 Task 查询/命令之上补充项目级 Task Key/标题搜索、完整祖先链，以及面向人类 Session 的 Task Workspace 当前清单和安全附件二进制读取。资源响应携带服务端计算的 `actions` 供客户端呈现；每次读取和写入仍重新解析会话、项目、Membership、Task、有效 Owner、Admin Mode、资源版本和影响集合，不能把按钮可见性、URL 或缓存当作授权。
...
M3 Web 使用 React 19 与 TanStack Query：同源 API/error/binary 层、按 user/project/scope/resource 分区的稳定 query key、一次用户意图一个幂等键，以及只在当前 React 内存保存的项目级 Admin Mode ID。中文界面在既有认证与项目治理上增加正式 Task 页面；所有写操作按服务端 `actions` 呈现，并在提交时重新携带版本、影响令牌和适用管理员上下文。SSE 只使权威 query 失效，不覆盖草稿或重放写入。既有 `?prototype=task-ui` 入口继续消费隔离夹具。
