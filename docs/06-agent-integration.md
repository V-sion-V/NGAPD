...
Agent 对当前用户具有写入资格并已取得租约的用户级或任务级工作区拥有用户内容完整访问权限。项目级工作区还必须由用户明确要求并确认进入管理员模式。上述文件操作无需逐次人工确认，但仍必须写入文件/同步审计并向用户报告结果。完整访问权不允许访问工作区外路径，不允许绕过唯一写入租约，也不允许写入 `.ngapd/`、`TASK.md` 或 `SUMMARY.md`。租约或版本失效后，本地改动直接由服务端版本替换，不能通过确认恢复提交资格。
...
| `ngapd.set_task_owner` | 原 Owner、新 Owner、待接受状态、活动租约和服务端最终同步版本 |
| `ngapd.accept_task_owner` | 新建指派、未分配任务指派或 Owner 转移请求，以及接受后获得的责任和工作区资格 |
...
| `ngapd.delete_task` | 仅非顶层任务；不可恢复删除的子树、工作区和依赖影响 |
...
服务端按作用域校验写入资格：用户级要求对应用户；项目级要求 Project Owner/Admin、用户明确提出本次项目工作区维护意图并已确认进入管理员模式；任务级要求 Task Owner。读取其他用户的用户级作用域还必须携带与当前用户指令绑定的读取意图。输出规范化作用域、实际访问模式、本地安全路径、Workspace Lease ID（如有）、文件 manifest 和同步状态。
...
`due_at` 必须是 UTC RFC 3339 时间点；`logical_role_id` 必须属于当前项目，归档角色不能新绑定。调用必须携带 `expected_version`，先生成字段级差异提案并由用户确认后才能执行。Owner、父节点、依赖、状态、阻塞和归档字段不能通过此工具夹带修改，它们使用各自同样需要确认的专用工具。

### 7.4 `split_task`

`create_task` 指定当前用户本人时可以在确认后直接创建；指定另一成员时必须创建待接受指派。顶层任务在目标成员接受前不分配 Task Key；普通子任务可以先创建为未分配，再等待目标成员接受。

输入应表达一组候选子任务及候选依赖。服务端在提案阶段：

- 验证当前用户能否在父任务下创建。
- 验证 Owner 是活动成员。
- 候选 Owner 不是当前用户时创建待接受指派；顶层任务在接受前不分配 Task Key，普通子任务可以先作为未分配任务创建。
...
如果提案过期，不保留或占用任务编号。拆分结果中指派给其他成员的普通子任务先以未分配状态创建，并分别产生 Owner 接受请求；依赖可以创建，但相关任务在有 Owner 前不能开始或完成。
...
| `workspace_lease_expired` | 当前租约无效 | 停止写入；重新连接时接受服务端版本替换本地改动 |
...
4. `create_task`、`split_task`、带 `graph_version` 和跨 Owner 接受的依赖工具、状态工具和管理员模式的提案确认。
5. `complete_task`、摘要状态和知识搜索。
6. 需接收者接受的 Owner 指派或转移，以及移动、顶层归档、非顶层不可恢复删除和管理员批量工具。
