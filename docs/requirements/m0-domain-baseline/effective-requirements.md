...
M0 已将 NGAPD 的工程骨架和 Workspace Sync、Task UI、Agent Context 前置原型所证明的约束收敛为可执行、可迁移、可验证的服务端领域基线。当前基线为 M1“项目、角色和成员”与 M2“任务管理闭环”提供统一的领域、数据库、应用和发布入口。
...
| FR-010 | Task 移动按稳定主键顺序锁定源/目标 graph scope，在锁内复核依赖、目标状态、树环、权限和版本；移动与依赖写共享互斥边界。 | AC-008 core | `requirements.md` |
...
| FR-016 | 移动、归档、删除、显式 Owner 变化、级联重开和 Follow 变化先计算稳定影响集合，覆盖适用的后代、依赖、状态、完成祖先、租约、未同步版本和 graph scope。 | AC-014 core | `requirements.md` |
| FR-017 | 授权只使用服务端解析的 user、membership、Project Owner、有效 Task Owner、admin session 与影响集合；确认不替代授权，租户查询包含服务端解析的 `project_id`。 | AC-007、AC-014–AC-015、AC-026 core | `requirements.md` |
...
| FR-022 | Task 完成、重开、Owner 变化、冻结、快照和 lease revoke 使用 Workspace 应用端口与事务协调，不留下 Task/Workspace 半状态；M0 不要求本地物化。 | AC-016 core | `requirements.md` |
...
| FR-028 | 不提前暴露完整 Project/Task CRUD、正式 Task UI、本地 Workspace 同步或 Agent 写工具；内部端口不能成为绕过未来授权/确认的公共入口。 | AC-024 core | `requirements.md` |
...
3. Task 在 Project 事务锁内分配下一个 Sequence 并组合 Task Key。
4. 并发或重试返回确定结果；回滚可留下间隙，但编号不复用。

### 3.3 修改依赖或移动 Task

1. 服务端解析 Project、scope、端点、Owner、membership 和当前版本。
2. 依赖变更按两端所有权直接执行或创建另一端 Owner 接受的请求。
3. 移动和依赖写按稳定 scope ID 顺序取得相同锁，在锁内重查授权、版本、树与图事实。
4. 成功操作原子更新结构/边、graph version、审计与 Outbox；失败不留半状态。

### 3.4 完成或重新打开 Task

1. completion 锁定并复核 Owner、子任务、predecessor、blocker、版本、授权与 Workspace。
2. 同一事务固化 Owner、冻结 Task/Workspace、创建快照、撤销 lease、写审计与 Outbox。
3. reopen 根据 `deny / cascade` 和确定影响集合执行，创建新 work cycle 并保留旧快照。
4. 任何 Owner、版本、lease 或影响确认漂移都稳定拒绝旧操作。
...
| AC-008 | core | 真实 PostgreSQL 下移动与依赖交错只形成一致前态或后态，无跨父边、树环、单边版本或半提交。 |
...
| AC-014 | core | 结构/Owner/reopen/Follow 操作返回稳定完整影响集合，授权和确认不遗漏适用事实。 |
| AC-015 | core | user/project/task Workspace 唯一；读写矩阵、唯一 lease、sync version、work cycle 和不可变版本保持。 |
| AC-016 | core | Task 生命周期与 Workspace 在并发和故障注入下无半状态。 |
...
| AC-020 | core | 客户端不导入服务端实现，生产领域规则可由 M1/M2 直接复用。 |
...
| AC-024 | core | 不存在完整 Project/Task 公共 CRUD、正式 Task UI、本地同步或 Agent 写工具。 |
...
当前 change-0 证据中 AC-001–AC-029 全部为 `passed`，没有开放 `FND-I-*`。

## 8. 当前决策

| 决策项 | 当前结论 | 来源 |
| --- | --- | --- |
| 交付与验证策略 | `relaxed`；core 和硬门禁阻塞，只有独立证明无交付影响的 supplemental 异常可 report-only | 用户确认 / `requirements.md` |
| 原型数据库数据 | 可丢弃；只允许对精确确认目标执行破坏性重建，不承诺保留或恢复 | 用户确认 / `requirements.md` |
| 工作流边界 | 一个 schema 3.2 `m0-domain-baseline` initial run，采用 `phased + expanded` 四阶段 | `workflow-contract.md` / `implementation-plan.md` |
...
没有被 change-0 删除或替换的产品需求。原始 FR-001–FR-028 与 AC-001–AC-029 全部继续生效。
