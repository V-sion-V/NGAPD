...
M0 已将 NGAPD 的工程骨架和 Workspace Sync、Task UI、Agent Context 前置原型所证明的约束收敛为可执行、可迁移、可验证的服务端领域基线。change-1 关闭子任务创建、移至虚拟根与 Owner 继承后代 Workspace 协调的授权/一致性缺口；change-2 进一步关闭 Follow/Blocker 低层授权与并发条件缺口，并明确 completion-ready 投影与 Owner 通知仍归 M2 交付。当前基线为 M1“项目、角色和成员”与 M2“任务管理闭环”提供统一的领域、数据库、应用和发布入口。
...
| FR-011 | Follow 只连接同项目不同 Task、同向唯一且可成环；只提供一跳发现，不改变 Owner、权限、依赖、状态、图版本或 Workspace 写资格。add/remove 只允许未完成、未归档 source 的有效 Owner，或消费有效管理员会话；Agent 管理能力要求用户显式请求。两端稳定加锁并绑定包含 source/target 的最新确定性影响确认，target 不因此成为写授权对象。 | AC-009、AC-014 core | `requirements.md`；change-2 RC-2-001 |
| FR-012 | 基础状态为 `not_started / in_progress / done`；`blocked` 由未解决人工 blocker 或未完成活动 predecessor 派生；直接子任务全部完成只产生父任务完成资格，不自动完成。M0 不交付幂等 `completion_ready` 投影、`TaskCompletionBecameReady` 事件或 Owner 通知，这些仍由 M2 交付。 | AC-010 core | `requirements.md`；change-2 RC-2-002、RC-2-003 |
...
| FR-016 | 移动、归档、删除、显式 Owner 变化、级联重开和 Follow 变化先计算稳定影响集合，覆盖适用的后代、依赖、状态、完成祖先、租约、未同步版本和 graph scope。Owner 变化的完整确认覆盖全部后代，并精确区分因显式 Owner 为空而实际继承目标 Owner 的受影响分支；Follow 的影响确认稳定包含 source/target，但不把 target Owner 变成写授权要求。 | AC-014 core | `requirements.md`；change-1 RC-002；change-2 RC-2-001 |
| FR-017 | 授权只使用服务端解析的 user、membership、Project Owner、有效 Task Owner、admin session 与影响集合；确认不替代授权，租户查询包含服务端解析的 `project_id`。内部创建子任务、Follow 和 Blocker 端口必须重新校验适用的有效 Owner/admin 与活动 actor；move-to-root 不得把虚拟根依赖控制权当作结构授权；Agent 管理能力必须来自用户显式请求。 | AC-007、AC-009、AC-010、AC-014–AC-015、AC-026 core | `requirements.md`；change-1 RC-001、RC-003；change-2 RC-2-001、RC-2-002 |
| FR-018 | 所有 M0 公共 HTTP 位于 `/api/v1`，使用 TypeBox 运行时 Schema 并进入 OpenAPI 3.1；可重试操作幂等，资源写使用资源版本、图版本或等价条件。内部 Follow command 绑定影响令牌，Blocker add 绑定 expected Task version 并在成功时递增 Task version。 | AC-017 core | `requirements.md`；change-2 RC-2-001、RC-2-002 |
...
| FR-028 | 不提前暴露完整 Project/Task CRUD、正式 Task UI、本地 Workspace 同步或 Agent 写工具；内部端口不能成为绕过未来授权/确认的公共入口，包括 Repository 子任务创建、Follow 和 Blocker 写入必须消费服务端授权上下文、确认或版本条件。 | AC-020、AC-024 core | `requirements.md`；change-1 RC-003；change-2 RC-2-001、RC-2-002 |
...
### 3.4 维护 Follow 或人工 Blocker

1. Follow preview 确认 source/target 是同项目不同 Task，source 未完成且未归档，并生成稳定包含两端的 `follow_change` 影响令牌。
2. Follow add/remove 稳定锁定两端，在锁内重读有效 source Owner、活动 actor 和管理员会话；普通模式只允许 source Owner，管理员可代管，Agent 管理能力要求用户显式请求。
3. Follow 提交重算影响令牌；漂移、重复 add 或不存在的 remove 稳定拒绝，成功只写关系、审计与 Outbox，不改变 Task/graph/Workspace 权威事实。
4. Blocker add 锁定 Task，在锁内重读有效 Owner/admin、活动 actor、生命周期和 expected Task version；成功原子递增 Task version、写 blocker、审计与 Outbox。
5. 已完成、冻结、归档、失活、越权或陈旧 Follow/Blocker 写入均不产生业务半状态。

### 3.5 Owner 变化、完成或重新打开 Task
...
- 公共 HTTP 继续使用 `/api/v1`、TypeBox、OpenAPI 3.1、request ID 与统一错误体；M0 新 Task 能力默认只作为内部应用/Repository 端口。内部 Follow command 包含影响令牌，Blocker command 包含 expected Task version。
...
- 非法 Project/Task Key、树环/孤儿/跨项目父子、跨父依赖、自环/重复边/DAG 环、缺失 Owner、冻结写、陈旧版本/请求/影响确认、无效 lease 与幂等冲突均返回稳定诊断；不存在的 Follow remove 同样稳定拒绝。
...
| AC-009 | core | Follow 同项目、同向唯一并只展开一跳；Follow 环不递归且不改变任何权威写资格或图事实。add/remove 只由未完成、未归档 source 的有效 Owner 或有效管理员会话执行，Agent 管理能力要求用户显式请求。 |
| AC-010 | core | blocked 仅由人工 blocker/活动 predecessor 派生；子 Task 完成只形成父 Task 完成资格且不自动完成。M0 不把 M2 的 completion-ready 投影或 Owner 通知误报为已交付。 |
| AC-011 | core | completion 仅在全部前置成立时提交，并同事务固化 Owner、冻结 Task/Workspace、创建快照与审计。 |
| AC-012 | core | 完成后的内容、Owner、结构、依赖、blocker、以其为 source 的 Follow 和 Task Workspace 写均拒绝，明确例外保留。 |
| AC-013 | core | deny 精确拒绝完成 successor；cascade 精确重开闭包并创建新 work cycle，保留旧快照并拒绝跨 Owner/stale 确认。 |
| AC-014 | core | 结构/Owner/reopen/Follow 操作返回稳定完整影响集合；Follow 确认包含 source/target 且提交时重算，Owner 变化确认全部后代并精确处理实际继承受影响分支，授权与确认不遗漏 Workspace/lease 事实。 |
...
| AC-017 | core | M0 公共 HTTP 均在 `/api/v1` 和 OpenAPI 3.1；内部 Follow/Blocker command 由共享 runtime Schema 校验，稳定错误体覆盖已公开和内部适配失败。 |
| AC-018 | core | 关键成功/失败均有不可变可查审计，字段完整且不泄露秘密或 Workspace 正文；Blocker 成功审计记录变更前后 Task version。 |
| AC-019 | core | Identity、Pairing、Workspace、Web 与 Workspace CLI 核心兼容继续通过。 |
| AC-020 | core | 客户端不导入服务端实现，生产领域规则可由 M1/M2 直接复用；Repository 子任务创建、Follow 和 Blocker 写入不能绕过适用的有效 Owner/admin、活动 actor、影响或版本条件。 |
...
| AC-024 | core | 不存在完整 Project/Task 公共 CRUD、正式 Task UI、本地同步或 Agent 写工具；内部创建、Follow 和 Blocker 端口不会成为授权、确认或版本绕过入口。 |
...
change-0 的 AC-001–AC-029 保持 `passed`；change-1 对 AC-008、AC-014、AC-016、AC-020、AC-024 的补充回归为 `passed`；change-2 对 AC-009、AC-010、AC-012、AC-014、AC-017、AC-018、AC-020、AC-024 的补充回归同样为 `passed`。没有开放 `FND-I-*`、`FND-C1-*` 或 `FND-C2-*`。

## 8. 当前决策

| 决策项 | 当前结论 | 来源 |
| --- | --- | --- |
| 交付与验证策略 | initial/change-0 使用 `relaxed`；change-1 与 change-2 均使用用户显式选择的 `strict`，测试先行且全部范围内门禁阻塞 | 用户确认 / `requirements.md` / change-1 / change-2 |
| 原型数据库数据 | 可丢弃；只允许对精确确认目标执行破坏性重建，不承诺保留或恢复 | 用户确认 / `requirements.md` |
| 工作流边界 | schema 3.2 `m0-domain-baseline` initial run 采用 `phased + expanded` 四阶段；change-1 和 change-2 各采用一个 compact P-001 纠正阶段 | `workflow-contract.md` / `implementation-plan.md` / change-1 / change-2 |
| AC-010 里程碑边界 | M0 证明 blocked 派生、完成资格和不自动完成；completion-ready 投影、事件与 Owner 通知保留到 M2 | `docs/07-roadmap-and-validation.md` / change-2 RC-2-003 |
...
没有被 change-0、change-1 或 change-2 删除或替换的产品需求。原始 FR-001–FR-028 与 AC-001–AC-029 全部继续生效。
...
change-2 的 RC-2-001–RC-2-003 均为 `modify/clarify`：分别补齐 Follow 的 source Owner/admin 与影响确认、Blocker 的 Owner/admin 与 Task version 条件，以及 AC-010 的 M0/M2 里程碑来源。它们没有删除一次可完成提示的产品能力；该能力仍由 M2 交付。
...
11. change-2 计划与执行：[`change-plan.md`](execution/change-2/change-plan.md)、[`phase-001-plan.md`](execution/change-2/phase-001-plan.md)、[`execution-state.md`](execution/change-2/execution-state.md)。
12. change-2 不可变阶段结果：[`phase-001-result.md`](execution/change-2/phase-001-result.md)，SHA-256 `04aee30e87e3e4fb1fabbe7b8461e375cd16befe91cf3315ae8d3ab55a918074`。
13. Follow/Blocker 与验收来源纠正记录：[`change-2.md`](change-2.md)。
