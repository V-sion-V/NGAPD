...
任务完成事务中，如果显式 Owner 为空，系统先把当前有效 Owner 固化到 `owner_membership_id`，确保之后祖先 Owner 改变不会改写已完成任务的责任归属。随后进入冻结边界：标题、正文、截止时间、逻辑角色、标签、展示类型、显式 Owner、父子关系、阻塞、依赖和任务级工作区都不得修改。仍允许活动项目成员添加评论、显式重新打开任务，以及对已完成顶层任务执行归档；摘要作为独立 KnowledgeEntry 继续按其版本规则修订。
