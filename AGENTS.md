...
- 已完成基线：三个前置原型、Workspace CLI 初始工作流，以及 M0“领域基线和工程骨架”、M1“项目、角色和成员”、M2“任务管理闭环”和 M3“平铺树状任务界面”的已记录运行均为 `completed/passed`。M2、M3 initial 历史和 M3 最佳实践工作流回顾已经封存。
- 当前开发状态：当前分支为 `codex/m4-workspace-sync-platform-adapter`，从已合并并推送的 `main@f032c67` 建立，产品里程碑已切换到 M4“Workspace 同步平台适配器”入口。M4 尚未建立 schema-v3 功能目录、需求、路线图或 execution state，也没有 M4 产品实现；下一且唯一允许的产品步骤是使用 `$clarify-feature-requirements` 检查项目事实、收集本次 strict/relaxed 策略及必要决策并建立独立工作流。
- M3 交付状态：M3 已按 schema-v3.2、`relaxed` 策略完成 initial P-001/P-002，并由 `change-0.md` 与 `effective-requirements.md` 冻结为 `completed/passed`。正式 Schema 仍为 version 3，最新迁移为 `0010-m3-task-ui-history-compatibility`；生产 React Task UI 已交付单 scope DAG、深链/History、搜索筛选、活动/归档浏览、完整 M2 人类操作、评论/附件/活动/通知、Admin Mode、SSE 草稿保护和安全通知导航。最终 Node 24/pnpm 11/PostgreSQL 17 根 `pnpm run ci` 为 311 tests passed、0 failed、9 platform-conditional skipped；M3 工作流回顾确认 2 个阶段、5 个任务、40 项 FR 与 28 项 AC 全部闭合，无开放 finding。
...
- 工作流边界：M2、M3 initial 历史已经冻结；任何对应需求或行为变化只能使用 `$apply-feature-change` 创建连续 change run。M4 必须先使用 `$clarify-feature-requirements` 建立独立 schema-v3 工作流，不得把分支存在误记为需求已确认、planning、ready 或 in-progress，也不得改写 M2/M3 result、`change-0.md` 或 effective snapshot。
- 阶段依据：[M3 当前有效需求](docs/requirements/m3-task-ui/effective-requirements.md)、[M3 初始实现记录](docs/requirements/m3-task-ui/change-0.md)、[M3 工作流回顾](docs/requirements/m3-task-ui/workflow-report.md)、[M3 P-002 结果](docs/requirements/m3-task-ui/execution/initial/phase-002-result.md)、[M3 参考服务器验证](docs/requirements/m3-task-ui/validation/reference-server-2026-07-31.md)、[M2 当前有效需求](docs/requirements/m2-task-management/effective-requirements.md)、[M2 初始实现记录](docs/requirements/m2-task-management/change-0.md)与[总体实施路线](docs/07-roadmap-and-validation.md)。活动运行或里程碑状态变化后必须立即更新本节。
